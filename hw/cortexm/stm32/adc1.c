...
static void stm32_adc_set_swstart_irqs(STM32ADCState *state, uint32_t old_cr2,
        uint32_t new_cr2)
{
    static uint8_t idx_adc_value = 0;
    ...
    uint32_t changed = old_cr2 ^ new_cr2;

    // Filter changed pins that are outputs - do not touch input pins.
    // Bit 30: SWSTART
    uint32_t changed_out = changed & (1<<30);
    ...
    if (changed_out) {
        ...
        peripheral_register_write_value(cr2, rst_value);
        printf("=== ADC1_CR2 register %x --> %x \n", new_cr2, rst_value);
    }
}



static void stm32f4_adc_cr2_post_write_callback(Object *reg, Object *periph,
        uint32_t addr, uint32_t offset, unsigned size,
        peripheral_register_t value, peripheral_register_t full_value)
{
    STM32ADCState *state = STM32_ADC_STATE(periph);
    ...
    uint32_t prev_value = peripheral_register_get_raw_prev_value(cr2);

    //printf("=== Enter ADC1_CR2 interrupt \n");
    //printf("=== ADC1_CR2 register %x --> %lx \n", prev_value, full_value);

    // 'value' may be have any size, use full_word.
    stm32_adc_set_swstart_irqs(state, prev_value, full_value);
}
...
static void stm32_adc_realize_callback(DeviceState *dev, Error **errp)
{
    ...
    case STM32_FAMILY_F4:
        ...
        if (capabilities->f4.is_40x)
        {
            ...
            // CR2 write callback
            peripheral_register_set_post_write(state->u.f4.reg.cr2, &stm32f4_adc_cr2_post_write_callback);
